    ...
    allPages: [
      {
        ...
        publishDate: new Date('2023-04-24').getTime(),
        publishDay: '2023-04-24',
        lastEditedDate: new Date('2023-04-24').toISOString(),
...
async function getDataBaseInfoByNotionAPI({ pageId, from }) {
  ...
  const collectionEntry = Object.entries(pageRecordMap.collection || {})[0]
  const collectionView = pageRecordMap.collection_view || {}
  const hasCollectionData = Boolean(collectionEntry)
  ...
  if (
    ...
    console.warn(
      `pageId "${pageId}" is not a legacy collection view; falling back to the first collection in the Notion response.`
    )
  }
  const collection = collectionEntry?.[1]?.value || {}
  const collectionId = rawMetadata?.collection_id || collectionEntry?.[0]
  const collectionQuery = pageRecordMap.collection_query
  const schema = collection?.schema

  const viewIds = rawMetadata?.view_ids || Object.keys(collectionView)
